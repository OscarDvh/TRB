...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const equiposReparacionEl = document.getElementById("equiposReparacion");
  ...
  // Función para generar número de orden con transacción
...
// === Generar PDF con estilo tipo formato físico ===
function generarOrdenPDF(orden) {
  ...
  logo.src = "./src/assets/logo.png"; // Ruta del logo en tu proyecto
  ...
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      const numeroOrden = await generarNumeroOrden(); // Generamos número
      const datos = {
  ...
  onSnapshot(clientesRef, (querySnapshot) => {
    ...
    if (querySnapshot.empty) {
      ...
      equiposReparacionEl.textContent = 0;
      clientesActivosEl.textContent = 0;
      return;
    }
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      tr.innerHTML = `
        ...
        <td>${c.descripcion ?? ""}</td>
        <td>
          <select class="select select-bordered select-sm estado-select" data-id="${
            docSnap.id
          }">
            <option value="Pendiente" ${
              c.estado === "Pendiente" ? "selected" : ""
            }>Pendiente</option>
            <option value="En progreso" ${
              c.estado === "En progreso" ? "selected" : ""
            }>En progreso</option>
            <option value="Entregado" ${
              c.estado === "Entregado" ? "selected" : ""
            }>Entregado</option>
          </select>
        </td>
        <td>
          ...
          <button class="btn btn-sm btn-primary btn-pdf" data-id="${
            docSnap.id
          }">PDF</button>
        </td>
      `;
      ...
      tr.querySelector(".btn-editar").addEventListener("click", () => {
        ...
        document.getElementById("editar-numeroDeSerie").value =
          c.numeroDeSerie ?? "";
        ...
        document.getElementById("editar-fechaEntrega").value =
          c.fechaEntrega ?? "";
      ...
      // Cambio de estado
      tr.querySelector(".estado-select").addEventListener(
        "change",
        async (e) => {
          ...
          try {
            ...
            console.log(
              `Estado actualizado a ${nuevoEstado} para la orden ${c.orden}`
            );
          } catch (err) {
